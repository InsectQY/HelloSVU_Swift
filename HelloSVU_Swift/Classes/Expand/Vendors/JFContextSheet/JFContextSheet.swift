...
class JFContextSheet: UIView {
    ...
    /**
     item触摸手势
     */
    @objc func didTappedItemWithItemName(_ tap: UITapGestureRecognizer) -> Void {
    ...
    func getCircleCoordinate(_ center: CGPoint, angle: CGFloat, radius: CGFloat) -> CGPoint {
        let x = radius * CGFloat(cosf(Float(angle) * Float(Double.pi) / 180))
        let y = radius * CGFloat(sinf(Float(angle) * Float(Double.pi) / 180))
    ...
    func startSpringAnimation(_ centerPoint: CGPoint) -> Void {
        ...
        for (index, item) in subviews.enumerated() {
            ...
            // 上
            if centerPoint.x > insetX && centerPoint.x <= ScreenW - insetX && centerPoint.y <= insetY {
            ...
            // 右上角
            if centerPoint.x > ScreenW - insetX && centerPoint.y <= insetY {
                pathRadius = cornerRadius
            ...
            // 左
            if centerPoint.x <= insetX && centerPoint.y > insetY && centerPoint.y <= ScreenH - insetY {
                pathRadius = sideRadius
                switch subviews.count {
                case 1:
                    startAngle = 90
            ...
            // 左下
            if centerPoint.x <= insetX && centerPoint.y > ScreenH - insetY {
                pathRadius = cornerRadius
            ...
            // 右
            if centerPoint.x > ScreenW - insetX && centerPoint.y > insetY && centerPoint.y <= ScreenH - insetY {
                pathRadius = sideRadius
                switch subviews.count {
                case 1:
                    startAngle = 180
            ...
            // 右下
            if centerPoint.x > ScreenW - insetX && centerPoint.y > ScreenH - insetY {
                pathRadius = cornerRadius
